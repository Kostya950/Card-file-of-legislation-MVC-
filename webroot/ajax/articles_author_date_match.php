...
if(isset($_POST['publisher']) OR isset($_POST['date'])){
    ...
    $date = $_POST['date'];
    $sql = "SELECT * FROM file_articles_all_info WHERE `publisher_1`='{$publisher}' AND `date` = '{$date}'
    OR `publisher_2`='{$publisher}'";
    $docs = $db->query($sql);
    if(isset($docs['0'])) {
        ?>
            ...
            <div class="panel panel-danger text-center">
                ...
                <div class="panel-body  text-left">
                    <?php foreach ($docs as $doc) {
                        ...
                        echo "<form method='get' action='/manage/admin/edit_articles/{$doc['id']}'><span class='text-muted'> змінити статтю № </span><input type='submit' class='btn btn-default btn-sm' value='{$doc['id']}'></form>";
                        echo"<br>";
                        echo"<br>";
                    } ?></div>
            </div>
